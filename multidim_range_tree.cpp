...
template<int D, class T = int, class W = long long>
struct RangeTreeND {
    using P = array<T, D>;
    using V = vector<pair<P, W>>;
    ...
    void add(const P& p, W w = 1) { pts.push_back({p, w}); }

    void build() { build(root, pts, 0); }

    void build(Tree& t, V& p, int d) {
        sort(p.begin(), p.end(), [&](const pair<P, W>& a, const pair<P, W>& b) {
    ...
    W go(const Tree& t, int v, int nl, int nr, int l, int r, int d, const P& lo, const P& hi) const {
        if (r <= nl || nr <= l) return W(0);
    ...
    W query(const Tree& t, int d, const P& lo, const P& hi) const {
    ...
    W query(const P& lo, const P& hi) const { return query(root, 0, lo, hi); }
};
